...
class Frame( object ):
    ...
    def __init__(   self                    ,
        ...
        # - PTX Len         = RANDOM
        ########################################################################################
        elif ( self.frame_type == 'n_key_ups_fixed'):
            # SOP
            self.sop_mode       = 'single'
            ...
            if ( aad_len <= msg_limit ):
                ...
                if ( self.aad_len > 0):
                    for i in range( 0, self.aad_len ):
                        (self.aad).append("ddddddddddddddddddddddddddddddddeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee")
                        print "AAD          [", i, "]       = ", (self.aad[i]).encode("hex")
        ...
        # - AAD Len         = RANDOM
        ########################################################################################
        elif ( self.frame_type == 'custom_len+iv_fixed'):
            # SOP
            self.sop_mode       = 'single'
        ...
        if ( self.ptx_len > 0 ):
            ...
            if ( self.ptx_mode == "random" ):
                while ( v < self.ptx_len ):
                    ...
                    if (1): print "PLAINTEXT    [", i, "]       = ", (self.plaintext[i]).encode("hex")
                    i = i + 1
            elif ( self.ptx_mode == "fixed" ):
                while ( v < self.ptx_len ):
                    if (debug): print "v ", v
                    if ( self.ptx_len%2 == 0 ):
                        if (debug): print "========================            EVEN            ========================"
                        (self.plaintext).append("1111111111111111111111111111111122222222222222222222222222222222")
                        v = v + 2
                    else:
                        if (debug): print "========================            ODD            ========================"
                        if ( v == self.ptx_len-1 ):
                            if (debug): print "========================            ULTIMO            ========================"
                            (self.plaintext).append("33333333333333333333333333333333")
                            v = v + 1
                        else:
                            if (debug): print "========================            ELSE_ULTIMO            ========================"
                            (self.plaintext).append("1111111111111111111111111111111122222222222222222222222222222222")
                ...
                raise Exception("Plaintext mode do not exist!!")
    ...
    def valid_generator( self ):
        ...
        # if( self.valid_mode == 'max_valid_fixed'    ):
        #     if ( self.reset_mode == 'null' ):
        #         self.valid      = [ 0 ]*1
        #     else:
        #         self.valid      = [ 0 ]*2
        #     while( valid_count <= self.data_time + 1500):
    ...
    def sop_generator( self ):
        ...
        valid_clk   = 0
        n_sops      = 0
        if ( self.sop_mode == 'null'                              ):
            self.sop      = [ 0 ]*len(self.valid)


        elif ( self.sop_mode == 'single'                              ):
            self.sop      = [ 0 ]*len(self.valid)
            if ( self.reset_mode == 'null'):
                self.sop[0]   = 1
            else:
    ...
    def time_wizard( self ):
        ...
        self.run_time           = self.ghash_done + 1500
...
class Basic_Frame_Sequencer( object ):
    ...
    def __init__(   self                        ,
        ...
        if  ( self.frame_seq_type == 'empty_frame' ):
            kup_time        = 130*8# + 44
            run_time_sum    = 0
            # INITIAL
            ########################################################################################
            ...
            for i in range( 0, self.m1_frames ):
                ...
                (self.initial_frame).append( Frame( frame_type='init', valid_mode='max_valid', gmac_mode=self.gmac_mode, key_mode='custom', key=key_std, iv=iv_std, ptx_len=4, aad_len=1 ) )
            ...
            for i in range( 0, self.m2_frames ):
                ...
                (self.custom).append( Frame( frame_type='custom_len+iv', valid_mode='max_valid', gmac_mode=self.gmac_mode, key_mode='custom', key=key_std, iv=iv_std, ptx_len=5, aad_len=1 ) )
            ...
            for i in range( 0, self.m2_frames ):
                print "Creating frame %d/%d..." % (i+1, self.m2_frames)
                (self.custom2).append( Frame( frame_type='custom_len+iv', valid_mode='max_valid', gmac_mode=self.gmac_mode, key_mode='custom', key=key_std2, iv=iv_std, ptx_len=5, aad_len=1 ) )
            ...
            for i in range( 0, 1 ):
                print "Creating KUP frame %d/%d..." % (i+1, 1)                                                                                 #os.urandom(32)
                frame   =   Frame( frame_type='n_key_ups_fixed', valid_mode='max_valid', gmac_mode=self.gmac_mode, key_mode='custom', key=key_std, new_key=key_std2, n_key_ups=1 )
            ...
            # - SOP Mode        = NULL
            # - RESET Mode      = NULL
            # - VALID Mode      = NULL < max_valid, random >
            # - GMAC Mode       = OFF < on, off >
            # - IV              = CUSTOM < argument iv >
            # - KEY Mode        = CUSTOM < argument key >
            # - KEY UP Mode     = NULL
            # - N KEY UPs       = 0
            # - PTX Mode        = NULL < argument ptx_len >
            # - PTX Len         = NULL
            # - AAD Mode        = NULL < argument aad_len >
            # - AAD Len         = NULL
            ########################################################################################
            self.empty_frame                = []
            for i in range( 0, self.m2_frames*20 ):
                print "Creating NULL frame %d/%d..." % (i+1, 1)
                frame   =   Frame( frame_type='empty', gmac_mode=self.gmac_mode, key=key_std)
                (self.empty_frame).append( frame )

            self.frame_seq      = [ self.initial_frame, self.custom, self.empty_frame, self.custom, self.empty_frame, self.custom,
                                    self.n_key_ups_frame, 
                                    self.custom2, self.empty_frame, self.empty_frame, self.empty_frame, self.empty_frame, self.custom2, self.empty_frame, self.custom2 ]

        elif  ( self.frame_seq_type == 'custom_len+iv' ):
            kup_time        = 130*8# + 44
            run_time_sum    = 0
            # INITIAL
            ########################################################################################
            ...
            for i in range( 0, self.m1_frames ):
                print "Creating frame %d/%d..." % (i+1, self.m1_frames)
                (self.initial_frame).append( Frame( frame_type='init', valid_mode='max_valid', gmac_mode=self.gmac_mode, key_mode='custom', key=key_std2, iv=iv_std, ptx_len=4, aad_len=1 ) )
            ...
            for i in range( 0, self.m2_frames ):
                print "Creating frame %d/%d..." % (i+1, self.m2_frames)
                (self.custom).append( Frame( frame_type='custom_len+iv', valid_mode='max_valid', gmac_mode=self.gmac_mode, key_mode='custom', key=key_std2, iv=iv_std, ptx_len=100, aad_len=100 ) )
            ...
            # for i in range( 0, self.m2_frames ):
            #     print "Creating frame %d/%d..." % (i+1, self.m2_frames)
            #     (self.custom2).append( Frame( frame_type='custom_len+iv', valid_mode='max_valid', gmac_mode=self.gmac_mode, key_mode='custom', key=key_std2, iv=iv_std, ptx_len=100, aad_len=100 ) )
            ...
            self.frame_seq      = [ self.initial_frame, self.custom, self.custom]#, self.n_key_ups_frame, self.custom2, self.empty_frame, self.empty_frame, self.empty_frame, self.empty_frame, self.custom2, self.empty_frame, self.custom2 ]

        elif  ( self.frame_seq_type == 'start1'):
            # INITIAL
            ########################################################################################
            # This Frame Type has the following characteristics:
            # - SOP Mode                        = SINGLE
...
def main():
    ...
    if ( generate_basic_seqs ):
        ...
        # custom_frame_seq            = Basic_Frame_Sequencer( frame_seq_type='empty_frame', m1_frames=1, m2_frames=1 )

        custom_frame_seq            = Basic_Frame_Sequencer( frame_seq_type='custom_len+iv', m1_frames=1, m2_frames=1 )
